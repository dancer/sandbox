...
test("blaxel maps create options and normalized operations", async () => {
  ...
  try {
    ...
    await expect(sandbox.ports.expose(15_500, host)).rejects.toMatchObject({
      code: "unsupported",
      provider: "blaxel",
    });
...
test.each(["legacy", "structured"])(
  "blaxel only maps %s missing paths to false",
  async (format) => {
    const original = SandboxInstance.create;
    const reads: string[] = [];
    const raw = {
      ...
      fs: {
        ls: (path: string) => {
          ...
          const status = path.endsWith("missing") ? 404 : 401;
          return Promise.reject(
            format === "legacy"
              ? new Error(JSON.stringify({ status }))
              : Object.assign(new Error("Sandbox request failed"), { status })
          );
        },
        mkdir: () => Promise.resolve({}),
        readBinary: (path: string) => {
